...
resource "aws_s3_bucket" "nextjs_bucket" {
  bucket = "kthread-portproj-bucket"
}
...
# Origin Access Identity - guarantees that only CloudFront can access s3 bucket
resource "aws_cloudfront_origin_access_identity" "origin_access_identity" {
  comment = "OAI for Next.JS portfolio site"
}

# Cloudfront distribution - this serves the content delivery network
